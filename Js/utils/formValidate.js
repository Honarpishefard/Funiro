...
function octaValidate(form_ID, userConfig) {
  ...
  const ovDoError = function (err) {
    if (!findElem("octavalidate-exception")) {
      ...
      errEl.innerText =
        "An Exception has occured. Please open the browser's console to preview & debug.";
  ...
  if (typeof form_ID === "undefined")
    ovDoError(
      "A valid Form Id must be passed as the first Argument during initialization"
    );
  ...
  if (typeof userConfig !== "undefined") {
    ...
    if (
      userConfig.strictWords !== undefined &&
      userConfig.strictMode !== undefined
    ) {
      if (userConfig.strictMode && userConfig.strictWords.length !== 0) {
        //merge both arrays but remove duplicates
        config.strictWords = [
          ...new Set([...config.strictWords, ...userConfig.strictWords]),
        ];
  ...
  (function (form_id) {
    ...
    const ovPrimaryValidations = document.querySelectorAll(
      `#${form_id} [octavalidate]`
    );
    //get all minlengths
    ...
    if (ovPrimaryValidations) {
      ...
      while (e < ovPrimaryValidations.length) {
        ...
        //get type
        const type = ovPrimaryValidations[e].getAttribute("type")
          ? ovPrimaryValidations[e].getAttribute("type")
          : null;
        //exit if attribute existts but id is null
        if (!id) {
          ovDoError(
            `One or more fields in ${form_id} does not have an id [Identifier] attached to it.`
          );
        }
        //["R", "required"], ["EMAIL", "email"], ["R", "radio"]
        validatePrimary[id] =
          validatePrimary[id] === undefined
            ? [attrVal, type]
            : [...validatePrimary[id], attrVal, type];
        //increment
        ++e;
      }
    }
    ...
    if (ovMinLengths) {
      ...
      while (e < ovMinLengths.length) {
        ...
        validateAttr[id] =
          validateAttr[id] === undefined
            ? ["MINLENGTH:" + val]
            : [...validateAttr[id], "MINLENGTH:" + val];
        ++e;
      }
    }
    ...
    if (ovEqualTo) {
      ...
      while (e < ovEqualTo.length) {
        ...
        const val = ovEqualTo[e].getAttribute("equalto")
          ...
          : ovDoError(
              'You must provide a form input ID on the "equalto" attribute'
            );
        if (!id) {
          ovDoError(
            `One or more fields in ${form_id} does not have an id [Identifier] attached to it.`
        ...
        if (ovEqualTo[e].type === "file")
          ovDoError(
            `In order to use the "equalto" attribute, this form input ${id} must not be of type "file"`
          );
    ...
    if (ovSize) {
      ...
      while (e < ovSize.length) {
        ...
        const val = ovSize[e].getAttribute("size")
          ...
          : ovDoError(
              'You must provide a valid digital storage on the "size" attribute'
            );
        if (!id) {
          ovDoError(
            `One or more fields in ${form_id} does not have an id [Identifier] attached to it.`
        ...
        if (ovSize[e].type !== "file")
          ovDoError(
            `In order to use the "size" attribute, this form input ${id} must be of type "file"`
          );
        validateAttr[id] =
          validateAttr[id] === undefined
            ? ["SIZE:" + val]
            : [...validateAttr[id], "SIZE:" + val];
        ++e;
      }
    }
    ...
    if (ovMinSize) {
      ...
      while (e < ovMinSize.length) {
        ...
        let val = ovMinSize[e].getAttribute("minsize")
          ...
          : ovDoError(
              'You must provide a valid digital storage on the "minsize" attribute'
            );
        if (!id) {
          ovDoError(
            `One or more fields in ${form_id} does not have an id [Identifier] attached to it.`
        ...
        if (ovMinSize[e].type !== "file")
          ovDoError(
            `In order to use the "minsize" attribute, this form input ${id} must be of type "file"`
          );
        validateAttr[id] =
          validateAttr[id] === undefined
            ? ["MINSIZE:" + val]
            : [...validateAttr[id], "MINSIZE:" + val];
        ++e;
      }
    }
    ...
    if (ovMaxSize) {
      ...
      while (e < ovMaxSize.length) {
        ...
        let val = ovMaxSize[e].getAttribute("maxsize")
          ...
          : ovDoError(
              'You must provide a valid digital storage on the "maxsize" attribute'
            );
        if (!id) {
          ovDoError(
            `One or more fields in ${form_id} does not have an id [Identifier] attached to it.`
        ...
        if (ovMaxSize[e].type !== "file")
          ovDoError(
            `In order to use the "maxsize" attribute, this form input ${id} must be of type "file"`
          );
        validateAttr[id] =
          validateAttr[id] === undefined
            ? ["MAXSIZE:" + val]
            : [...validateAttr[id], "MAXSIZE:" + val];
        ++e;
      }
    }
    ...
    if (ovFiles) {
      ...
      while (e < ovFiles.length) {
        ...
        let val = ovFiles[e].getAttribute("files")
          ...
          : ovDoError(
              'You must provide the number of files to be selected on the "files" attribute'
            );
        if (!id) {
          ovDoError(
            `One or more fields in ${form_id} does not have an id [Identifier] attached to it.`
        ...
        if (!ovFiles[e].multiple)
          ovDoError(
            `In order to use the "files" attribute, this form input ${id} must have the attribute "multiple"`
          );
        //check if type is file
        if (ovFiles[e].type !== "file")
          ovDoError(
            `In order to use the "files" attribute, this form input ${id} must be of type "file"`
          );
        validateAttr[id] =
          validateAttr[id] === undefined
            ? ["FILES:" + val]
            : [...validateAttr[id], "FILES:" + val];
        ++e;
      }
    }
    ...
    if (ovMinFiles) {
      ...
      while (e < ovMinFiles.length) {
        ...
        let val = ovMinFiles[e].getAttribute("minfiles")
          ...
          : ovDoError(
              'You must provide the minimum files to be selected on the "minfiles" attribute'
            );
        if (!id) {
          ovDoError(
            `One or more fields in ${form_id} does not have an id [Identifier] attached to it.`
        ...
        if (ovMinFiles[e].type !== "file")
          ovDoError(
            `In order to use the "minfiles" attribute, this form input ${id} must be of type "file"`
          );
        validateAttr[id] =
          validateAttr[id] === undefined
            ? ["MINFILES:" + val]
            : [...validateAttr[id], "MINFILES:" + val];
        ++e;
      }
    }
    ...
    if (ovMaxFiles) {
      ...
      while (e < ovMaxFiles.length) {
        ...
        let val = ovMaxFiles[e].getAttribute("maxfiles")
          ...
          : ovDoError(
              'You must provide the maximum files to be selected on the "maxfiles" attribute'
            );
        if (!id) {
          ovDoError(
            `One or more fields in ${form_id} does not have an id [Identifier] attached to it.`
        ...
        if (ovMaxFiles[e].type !== "file")
          ovDoError(
            `In order to use the "maxfiles" attribute, this form input ${id} must be of type "file"`
          );
        validateAttr[id] =
          validateAttr[id] === undefined
            ? ["MAXFILES:" + val]
            : [...validateAttr[id], "MAXFILES:" + val];
        ++e;
      }
    }
    ...
    if (ovAccept) {
      ...
      while (e < ovAccept.length) {
        ...
        let val = ovAccept[e].getAttribute("accept")
          ...
          : ovDoError(
              'You must provide the file extensions to be matched against on the "accept" attribute'
            );
        if (!id) {
          ovDoError(
            `One or more fields in ${form_id} does not have an id [Identifier] attached to it.`
        ...
        if (ovAccept[e].type !== "file")
          ovDoError(
            `In order to use the "accept" attribute, this form input ${id} must be of type "file"`
          );
        validateAttr[id] =
          validateAttr[id] === undefined
            ? ["ACCEPT:" + val]
            : [...validateAttr[id], "ACCEPT:" + val];
        ++e;
      }
    }
    ...
    if (ovAcceptMime) {
      ...
      while (e < ovAcceptMime.length) {
        ...
        let val = ovAcceptMime[e].getAttribute("accept-mime")
          ...
          : ovDoError(
              'You must provide the file MIME types to be matched against on the "accept-mime" attribute'
            );
        if (!id) {
          ovDoError(
            `One or more fields in ${form_id} does not have an id [Identifier] attached to it.`
        ...
        if (ovAcceptMime[e].type !== "file")
          ovDoError(
            `In order to use the "accept-mime" attribute, this form input ${id} must be of type "file"`
          );
        validateAttr[id] =
          validateAttr[id] === undefined
            ? ["ACCEPT-MIME:" + val]
            : [...validateAttr[id], "ACCEPT-MIME:" + val];
        ++e;
      }
    }
  ...
  function moreCustomRules(rules) {
    if (!isObject(rules))
      ovDoError(
        "The rules provided must be a valid Object! Please refer to the documentation"
      );
  ...
  function validate() {
    ...
    if (!findElem(form_id))
      ovDoError(
        `Form Input Element [${form_id}] does not exist in the Browser DOM`
      );
        ...
        formInputs.forEach((input) => {
          ...
          if (input.id !== "") {
            ...
            if (config.strictMode === true) {
              ...
              let validationText;
              let elem = document.querySelector("#" + formInputId)
                ? document.querySelector("#" + formInputId)
                : null;

              if (elem === null) {
                ovDoError(
                  `The Form Input Element with the ID [${formInputId}] is nowhere to be found`
                );
              }
              //set strict words
              ...
              if (
                ...
                validationText = elem.getAttribute("ov-strict:msg")
                  ...
                  : `Please remove or replace '${checkStrictWords(
                      elem.value
                    ).join(", ")}'`;
                ...
                if (elem.addEventListener) {
                  elem.addEventListener("change", function () {
                    if (
                      this.value &&
                      checkStrictWords(this.value).length !== 0
                    ) {
                      errors[formInputId]++;
                      ovRemoveSuccess(index);
                      ovNewError(index, validationText);
                ...
                validatePrimary[index][0].split(",").forEach((item) => {
                  ...
                  if (
                    ...
                    if (octaValidations.ValidateEmail(elem.value) === false) {
                      ...
                      errors[formInputId]++;
                      validationInfo = `${
                        octaValidations.ValidateEmail(elem.value) === false
                      }`;
                  ...
                  if (
                    ...
                    if (!octaValidations.ValidateAlpha_Only(elem.value)) {
                      errors[formInputId]++;
                      validationInfo =
                        "octaValidations.ValidateAlpha_Only(elem.value)";
                  ...
                  if (
                    ...
                    if (!octaValidations.ValidateAlpha_Spaces(elem.value)) {
                      errors[formInputId]++;
                      validationInfo =
                        "octaValidations.ValidateAlpha_Spaces(elem.value)";
                  ...
                  if (
                    ...
                    if (!octaValidations.ValidateAlpha_Numeric(elem.value)) {
                      errors[formInputId]++;
                      validationInfo =
                        "octaValidations.ValidateAlpha_Numeric(elem.value)";
                  ...
                  if (
                    ...
                    if (!octaValidations.ValidateLower_Alpha(elem.value)) {
                      errors[formInputId]++;
                      validationInfo =
                        "octaValidations.ValidateLower_Alpha(elem.value)";
                  ...
                  if (
                    ...
                    if (!octaValidations.ValidateUpper_Alpha(elem.value)) {
                      errors[formInputId]++;
                      validationInfo =
                        "octaValidations.ValidateUpper_Alpha(elem.value)";
                  ...
                  if (
                    ...
                    if (!octaValidations.ValidatePWD(elem.value)) {
                      errors[formInputId]++;
                      validationInfo =
                        "octaValidations.ValidatePWD(elem.value)";
                  ...
                  if (
                    ...
                    if (!octaValidations.ValidateUrl(elem.value)) {
                      errors[formInputId]++;
                      validationInfo =
                        "octaValidations.ValidateUrl(elem.value)";
                  ...
                  if (
                    ...
                    if (!octaValidations.ValidateUrl_QP(elem.value)) {
                      errors[formInputId]++;
                      validationInfo =
                        "octaValidations.ValidateUrl_QP(elem.value)";
                  ...
                  if (
                    ...
                    if (!octaValidations.ValidateDate_MDY(elem.value)) {
                      errors[formInputId]++;
                      validationInfo =
                        "octaValidations.ValidateDate_MDY(elem.value)";
                  ...
                  if (
                    ...
                    if (!octaValidations.ValidateUserName(elem.value)) {
                      errors[formInputId]++;
                      validationInfo =
                        "octaValidations.ValidateUserName(elem.value)";
                  ...
                  if (
                    ...
                    if (!octaValidations.ValidateTEXT(elem.value)) {
                      errors[formInputId]++;
                      validationInfo =
                        "octaValidations.ValidateTEXT(elem.value)";
                ...
                validateAttr[formInputId].forEach((f) => {
                  ...
                  if (f.split(":")[0] === "ACCEPT" && type == "file") {
                      ...
                      for (let ef of elem.files) {
                        ...
                        if (
                          !requiredFileExts.includes(
                            ef.name
                              .toLowerCase()
                              .substring(ef.name.toLowerCase().lastIndexOf("."))
                          )
                        ) {
                          errors[formInputId]++;
                    ...
                    if (
                      ...
                      for (let ef of elem.files) {
                        ...
                        if (
                          ...
                          !requiredFileMime.includes(
                            ef.type.split(
                              ef.type.substr(ef.type.indexOf("/"))
                            )[0] + "/*"
                          )
                        ) {
                          errors[formInputId]++;
                          ...
                          validationText = elem.getAttribute(
                            "ov-accept-mime:msg"
                          )
                    ...
                    if (elem.files !== null && elem.files.length !== 0) {
                      ...
                      if (uploadedSize !== requiredSize) {
                        ...
                        validationText = elem.getAttribute("ov-totalsize:msg")
                          ...
                          : "Please select files that is equal to " +
                            f.split(":")[1].replace(" ", "").toUpperCase();
                        ovRemoveSuccess(index);
                        ovNewError(index, validationText);
                        if (elem.addEventListener) {
                    ...
                    if (!(elem.value.length >= Number(f.split(":")[1]))) {
                      errors[formInputId]++;
                      validationInfo = `${!(
                        elem.value.length >= Number(f.split(":")[1])
                      )}`;
